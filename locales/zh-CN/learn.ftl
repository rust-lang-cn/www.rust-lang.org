...
learn-book = <strong>《Rust 程序设计语言》</strong>被亲切地称为“圣经”。本书从基本原则出发，给出了
        Rust 语言的概览。您会在阅读本书时构建一些项目。在读完本书后，您将会坚实地掌握 Rust 语言。
learn-book-button = 阅读本书！
learn-rustlings = 此外，Rustlings 课程会指导您下载并设置 Rust 工具链，在命令行中教您阅读和编写
        Rust 代码的语法基础。它可以运行在您自己的环境中，是《通过例子学 Rust》之外的另一种选择。
learn-rustlings-button = 学习 Rustlings 课程！
learn-rbe = 如果您不喜欢阅读大量的文档来学习语言，那么《通过例子学 Rust》就能涵盖您要学的知识。
        虽然本书花了很多篇幅来解释代码，但它展示的代码很丰富，并且尽量减少了文字解释。
        它还包括很多练习！
learn-rbe-button = 阅读《通过例子学 Rust》！

learn-use = 随着 Rust 一起成长
...
learn-setup-heading = 快速开始
learn-setup = 快速配置 Rust 开发环境并编写一个小应用！

learn-install-heading = 安装 Rust
learn-play = 您可以在 Rust 练习场上在线试用 Rust 而无需在计算机上安装任何东西。
learn-play-button = 无需安装，直接试用 Rust

learn-install-rustup-header = Rustup：Rust 的安装程序和版本管理工具
learn-install-rustup = 安装 Rust 的主要方式是使用 Rustup 工具链，它是 Rust 的安装程序和版本管理工具。
...
learn-install-fmt-clippy = 您可以用 <code>rustup component add rustfmt</code>
        安装代码格式化工具 Rustfmt，用 <code>rustup component add clippy</code> 安装 lint 工具 Clippy。


learn-generating = 创建新项目
learn-generating-steps = <p>我们将在新的 Rust 开发环境中编写一个小应用。首先用 Cargo
    创建一个新项目。在您的终端中执行：</p>
...
learn-dependencies-steps = <p>现在我们来为应用添加依赖。您可以在
...
learn-app-steps = <p>现在我们用新的依赖库编写一个小应用。在 <code>main.rs</code> 中添加以下代码：</p>
    { $code }
...
learn-read-docs = 您已经是一名 Rustacean 了！欢迎！我们很高兴您的加入！当您准备好后，跳到学习页面，
                  您可以在那里找到大量的文档，它们帮助您继续您的 Rust 旅程。
learn-read-docs-button = 了解更多！

learn-ferris-who = 螃蟹 Ferris 是谁？
learn-ferris = <p>Ferris 是 Rust 社区的非官方吉祥物。很多 Rust 程序员自称“Rustaceans”，
