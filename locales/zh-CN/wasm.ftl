...
wasm-why-rust-small-code-heading = 代码尺寸更小
wasm-why-rust-small-code-description =
        代码尺寸越小，页面加载速度就越快。Rust 生成的 <code>.wasm</code> 模块不含类似于垃圾回收器这样的额外成本。高级优化和 tree shaking 优化可移除无用代码。
...
wasm-get-started-wasm-description =
        了解更多关于快速、安全和开放的WebAssembly虚拟机信息，并阅读其标准。
wasm-get-started-wasm-link = 了解更多

wasm-get-started-book-alt = wasm 和 Rust吉祥物
...
wasm-js-augment-description =
        WebAssembly 的梦想不是扼杀 JavaScript，而是与其并存，来帮助处理繁重或底层的一些任务。Rust以性能为先，这些任务也可以受益。
...
wasm-production-mozilla-quote =
        {"["}<code>source-map</code> 库的] JavaScript 实现以性能为由累积了大量的复杂代码，
        而我们将它替换为地道的 Rust。Rust 不会强迫我们在明确地表达意图和运行时性能之间做出选择。
...
wasm-production-dropbox-quote =
        {"["}Rust 的]特性使然，通过 WASM 可以轻松地将 DivANS 编解码器嵌入到网页中，如上所示
